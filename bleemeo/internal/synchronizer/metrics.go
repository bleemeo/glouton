...
func (s *Synchronizer) metricDeactivate(localMetrics []types.Metric) error {
	metricsKeepDeactivateDays := time.Duration(s.option.Config.Bleemeo.Cache.MetricsKeepDeactivateDays)
	...
	for k, v := range registeredMetrics {
		...
		if !v.DeactivatedAt.IsZero() {
			if s.now().Sub(v.DeactivatedAt) > metricsKeepDeactivateDays*24*time.Hour { //nolint: durationcheck
