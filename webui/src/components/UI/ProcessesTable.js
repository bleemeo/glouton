...
class ProcessesTable extends React.PureComponent {
  ...
  _handleDisplayAll = ({ page, onSizePerPageChange }) => {
    this.setState(
      ...
      () => {
        onSizePerPageChange(this.state.displayAll ? this.props.data.length : 20, page)
      }
    )
  }
  ...
  render() {
    ...
    const { sizePage, data, widthLastColumn, borderless, expandRow, classNames, renderLoadMoreButton } = this.props
    const columns = [
      {
        dataField: 'pid',
        text: 'PID',
      ...
      },
      {
        ...
        onSort: this.onSort,
        sortCaret: this.renderSortCarets,
        formatter: (cell, row) => <div>{cell && !isNaN(cell) ? formatToBytes(cell * 1000).join(' ') : ''}</div>,
        headerStyle: { width: '5rem' }
      },
      {
        dataField: 'status',
        ...
        formatter: (cell, row) => (cell === '?' || cell === 'idle' || cell === 'disk-sleep' ? 'sleeping' : cell),
        sort: true,
        onSort: this.onSort,
        sortCaret: this.renderSortCarets,
        headerClasses: 'text',
      ...
      },
      {
        ...
        formatter: (cell, row) => {
          return !isNullOrUndefined(cell) && !isNaN(cell) ? <GraphCell value={cell} /> : null
        },
        sort: true,
        onSort: this.onSort,
        sortCaret: this.renderSortCarets,
            ...
            <div>
              <div className="d-flex justify-content-center text-truncate borderless">
                <BootstrapTable
                  ...
                  rowClasses={cn('rowHeightReduced', { borderless: borderless })}
                  rowStyle={{ color: '#000' }}
                  bordered={false}
                  bootstrap4
                  hover
...
export const formatCmdLine = (input, widthLastColumn, expandable = false) => {
  ...
  return (
    <div
      className="cellEllipsis"
      style={{ maxWidth: widthLastColumn ? widthLastColumn + 'rem' : '59rem', width: 'auto' }}
    >
      ...
      {args.map((arg, idx) => {
        if (regexpEqual.test(arg)) {
          ...
          return (
            ...
            >
              {' '}
              {splittedArg[0]}=<span style={{ color: cmdLineCommand[2] }}>{splittedArg[1]}</span>
            </span>
          )
