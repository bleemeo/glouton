...
export const Problems = ({ problems }) => (
  <div>
    <ul className="list-unstyled mb-0">
      {problems ? problems.map((problem, idx) => <li key={idx}>{problem}</li>) : null}
    </ul>
  </div>
)
  ...
  return (
    <div>
      {descriptionPieces.map((descr, idx) => (
        <span key={idx}>
          {descr}
          {idx < descriptionPieces.length - 1 ? <span>&nbsp;&nbsp;&nbsp;</span> : null}
        </span>
      ))}
    </div>
  )
...
export const getThemeBackgroundColorsSelect = theme =>
  isDarkTheme(theme) ? ['#045FB4', '#0B3861', '#3a3a3a'] : ['#A9D0F5', '#EFF5FB', '#FFF']

export const computeStart = (type, period) => {
  if (chartTypes[0] === type) {
    if (period.minutes) {
      return new Date(new Date().setMinutes(new Date().getMinutes() - 1)).toISOString()
    } else {
      const end = new Date(period.to)
      return new Date(new Date().setMinutes(end.getMinutes() - 1)).toISOString()
    }
  } else {
    if (period.minutes) {
      return new Date(new Date().setMinutes(new Date().getMinutes() - period.minutes)).toISOString()
    } else {
...
export const fillEmptyPoints = (data, period) => {
  ...
  if (new Date(firstData[0]) > new Date(start)) {
    for (
      let iDate = new Date(new Date(firstData[0]).setSeconds(new Date(firstData[0]).getSeconds() - resampling / 6));
      iDate > new Date(start);
      iDate.setSeconds(iDate.getSeconds() - resampling / 6)
    ) {
      data.splice(0, 0, [new Date(iDate.setMilliseconds(0)).toISOString(), null])
    }
  ...
  return data.filter(d => new Date(d[0]) >= new Date(start) && new Date(d[0]) <= new Date(end))
}
...
export const computeBackwardForward = (
  ...
  const applyBackwardOrForward = isForward ? nbMinutes * 0.9 : 0 - nbMinutes * 0.9
