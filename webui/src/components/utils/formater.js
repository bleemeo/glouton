...
if (window.Intl) {
  ...
  dayFormater = new Intl.DateTimeFormat('en', { year: undefined, month: undefined, day: '2-digit' }).format
  // the month in English on 3 letters: Jan, Feb...
  monthFormater = new Intl.DateTimeFormat('en', { year: undefined, month: 'short', day: undefined }).format
  // The fullname month in English
  fullMonthFormater = new Intl.DateTimeFormat('en', { year: undefined, month: 'long', day: undefined }).format
  // 2 digits year
  yearFormater = new Intl.DateTimeFormat('en', { year: '2-digit', month: undefined, day: undefined }).format
  // 2 digit month
  monthFormater2Digit = new Intl.DateTimeFormat('en', { year: undefined, month: '2-digit', day: undefined }).format
  // 4 digits year
  yearFormater4Digit = new Intl.DateTimeFormat('en', { year: 'numeric', month: undefined, day: undefined }).format
  // hours & minutes follow the locale (17:05 for Europe, 5:05 PM for US)
  hoursMinutesFormater = new Intl.DateTimeFormat(window.navigator.language || 'en', {
    year: undefined,
    month: undefined,
    day: undefined,
    hour: '2-digit',
  ...
  hoursMinutesSecondsFormater = new Intl.DateTimeFormat(window.navigator.language || 'en', {
    year: undefined,
    month: undefined,
    day: undefined,
    hour: '2-digit',
...
export const percentToString2Digits = percent => {
  const formatter = Math.trunc(percent) > 0 ? d3FormaterHandlingNull('.2r') : d3FormaterHandlingNull('.1f')
  return formatter(percent) + ' %'
}
...
export const formatDateAgo = date => {
  const dateTS = date instanceof Date ? date.getTime() : new Date(date).getTime()
...
export const formatMetricName = metricName => {
  ...
  metricNameSplitted.forEach(metricNamePart => {
    result += metricNamePart.substring(0, 1).toUpperCase() + metricNamePart.substring(1) + ' '
  })
  return result
}
