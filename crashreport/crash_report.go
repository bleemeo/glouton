...
func bundleCrashReportFiles(ctx context.Context, maxReportCount int, stateDir string, enabled bool, diagnosticFn diagnosticFunc) (reportPath string) {
	...
	if err == nil || errors.Is(err, io.EOF) {
		if bytes.Contains(lastStderrFileContent, []byte("panic:")) || bytes.Contains(lastStderrFileContent, []byte("fatal error:")) || bytes.Contains(lastStderrFileContent, []byte("DATA RACE")) {
			foundStderrLog = true
		}
	}
