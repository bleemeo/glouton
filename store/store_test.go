...
func TestDropOldMetrics(t *testing.T) {
	...
	t0 := time.Now()
	testMinTime := t0.Add(-time.Hour)
	testMaxTime := t0.Add(time.Hour)

	tests := []struct {
		name       string
		pushPoints []types.MetricPoint
		now        time.Time
		dropMaxAge time.Duration
		want       []metricWant
	}{
		{
		...
		},
		{
			...
			pushPoints: []types.MetricPoint{
				{
					Point:  types.Point{Time: t0.Add(-7 * time.Minute), Value: 5},
					Labels: map[string]string{types.LabelName: "metric1"},
				},
			},
			now:        t0,
		...
		},
		{
			...
			pushPoints: []types.MetricPoint{
				...
				{
					Point:  types.Point{Time: t0.Add(-6 * time.Minute), Value: 5},
					Labels: map[string]string{types.LabelName: "metric3"},
				},
				{
					Point:  types.Point{Time: t0.Add(-2 * time.Minute), Value: 5},
					Labels: map[string]string{types.LabelName: "metric4"},
				},
			},
			now:        t0,
			dropMaxAge: 5 * time.Minute,
			want: []metricWant{
				...
				{
					labels: map[string]string{types.LabelName: "metric4"},
					points: []types.Point{{Time: t0.Add(-2 * time.Minute), Value: 5}},
				},
			},
		},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			store.PushPoints(context.Background(), test.pushPoints)

			store.DropOldMetrics(test.dropMaxAge)
			...
			if len(test.want) != store.MetricsCount() {
				t.Errorf("MetricsCount() = %d, want %d", store.MetricsCount(), len(test.want))
			}
		})
	}
}
