...
func validateServices(services []config.Service, logProcessingCfg config.OpenTelemetry) (map[NameInstance]config.Service, prometheus.MultiError) {
	...
	for _, srv := range services {
		...
		if logProcessingCfg.Enable && logProcessingCfg.AutoDiscovery.Enable && logProcessingCfg.AutoDiscovery.DiscoverContainerAndService {
...
func (d *Discovery) updateDiscovery(ctx context.Context, now time.Time) (time.Time, error) {
	...
	if d.logProcessingCfg.Enable && d.logProcessingCfg.AutoDiscovery.Enable && d.logProcessingCfg.AutoDiscovery.DiscoverContainerAndService {
