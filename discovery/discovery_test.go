...
func TestDiscoverySingle(t *testing.T) {
	...
	for i, c := range cases {
		...
		disc := New(&MockDiscoverer{result: []Service{c.dynamicResult}}, nil, nil, nil, state, nil, mockContainerInfo{}, nil, nil, nil, facts.ContainerFilter{}.ContainerIgnored, types.MetricFormatBleemeo)
...
func TestUpdateMetricsAndCheck(t *testing.T) {
	...
	state := mockState{}
	disc := New(mockDynamic, fakeCollector, nil, nil, state, nil, nil, nil, nil, nil, facts.ContainerFilter{}.ContainerIgnored, types.MetricFormatBleemeo)
