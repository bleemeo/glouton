...
// be immediately run.
func NewSMTP(address string, persitentAddresses []string, labels map[string]string, acc accumulator) *SMTPCheck {

	sc := &SMTPCheck{
		mainAddress: address,
	}
	sc.baseCheck = newBase("", persitentAddresses, true, sc.doCheck, labels, acc)
	return sc
}
