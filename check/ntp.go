...
// be immediately run.
func NewNTP(address string, persitentAddresses []string, labels map[string]string, acc accumulator) *NTPCheck {

	nc := &NTPCheck{
		mainAddress: address,
	}
	nc.baseCheck = newBase("", persitentAddresses, true, nc.doCheck, labels, acc)
	return nc
}
